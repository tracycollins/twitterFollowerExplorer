...
const optionDefinitions = [enableStdin, quitOnComplete, quitOnError, testMode];

const commandLineConfig = cla(optionDefinitions);

console.log(
  chalkInfo("UBI | COMMAND LINE CONFIG\n" + jsonPrint(commandLineConfig))
);
...
async function unionInputSets(params) {
  ...
  for (const inputsId of params.parents) {
    ...
    try {
      ...
      if (inputsObj) {
        console.log(
          chalkLog("UBI | UNION INPUT SET FOUND | " + inputsObj.inputsId)
        );
      } else {
      ...
      for (const type of Object.keys(inputsObj.inputs)) {
        newInputsObj.inputs[type] = _.union(
          newInputsObj.inputs[type],
          inputsObj.inputs[type]
        ).sort();

        if (newInputsObj.meta.type[type] === undefined) {
        ...
        console.log(
          chalkLog(
            "UBI | UNION INPUT SETS" +
              " | TYPE: " +
              type +
              " | PARENT " +
              inputsObj.inputs[type].length +
              " | CHILD " +
              newInputsObj.inputs[type].length
          )
        );

        if (newInputsObj.inputsMinimum[type] === undefined) {
  ...
  if (newInputsObj.meta.type === undefined) {
    throw new Error(
      "newInputsObj.meta.type UNDEFINED | " + newInputsObj.inputsId
    );
  }

  for (const type of Object.keys(newInputsObj.meta.type)) {
  ...
  try {
    ...
    printInputsObj(
      "UBI | +++ SAVED NETWORK INPUTS DB DOCUMENT",
      savedNetworkInputsDoc
    );

    console.log(
      chalkInfo(
        "UBI | ... SAVING INPUTS FILE: " +
          defaultInputsFolder +
          "/" +
          newInputsObj.inputsId +
          ".json"
    ...
    saveFileQueue.push({
      ...
      file: newInputsObj.inputsId + ".json",
      obj: newInputsObj,
    });
    return newInputsObj;
  } catch (err) {
    console.log(
      chalkError("UBI | *** CREATE NETWORK INPUTS DB DOCUMENT: " + err)
    );
    throw err;
  }
}
...
async function connectDb() {
  try {
    ...
    const db = await global.wordAssoDb.connect(
      MODULE_ID_PREFIX + "_" + process.pid
    );
    ...
    db.on("error", async function (err) {
      ...
      console.log(
        chalkError(MODULE_ID_PREFIX + " | *** MONGO DB CONNECTION ERROR")
      );
      db.close();
    ...
    db.on("close", async function (err) {
      ...
      statsObj.dbConnectionReady = false;
      console.log(
        chalkError(MODULE_ID_PREFIX + " | *** MONGO DB CONNECTION CLOSED")
      );
    ...
    db.on("disconnected", async function () {
      ...
      statsObj.dbConnectionReady = false;
      console.log(
        chalkAlert(MODULE_ID_PREFIX + " | *** MONGO DB DISCONNECTED")
      );
...
async function loadConfigFile(params) {
  ...
  try {
    ...
    if (loadedConfigObj.UBI_INPUTS_FILE_PREFIX !== undefined) {
      console.log(
        "UBI | LOADED UBI_INPUTS_FILE_PREFIX: " +
          loadedConfigObj.UBI_INPUTS_FILE_PREFIX
      );
    ...
    if (loadedConfigObj.UBI_TEST_MODE !== undefined) {
      console.log(
        "UBI | LOADED UBI_TEST_MODE: " + loadedConfigObj.UBI_TEST_MODE
      );
    ...
    if (loadedConfigObj.UBI_QUIT_ON_COMPLETE !== undefined) {
      console.log(
        "UBI | LOADED UBI_QUIT_ON_COMPLETE: " +
          loadedConfigObj.UBI_QUIT_ON_COMPLETE
      );
    ...
    if (loadedConfigObj.UBI_ENABLE_STDIN !== undefined) {
      console.log(
        "UBI | LOADED UBI_ENABLE_STDIN: " + loadedConfigObj.UBI_ENABLE_STDIN
      );
    ...
    if (loadedConfigObj.UBI_KEEPALIVE_INTERVAL !== undefined) {
      console.log(
        "UBI | LOADED UBI_KEEPALIVE_INTERVAL: " +
          loadedConfigObj.UBI_KEEPALIVE_INTERVAL
      );
...
const loadInputsDropbox = async (params) => {
  ...
  console.log(
    chalkLog(
      "UBI | LOADING DROPBOX BEST INPUTS CONFIG | " +
        folder +
        "/" +
        file +
        " ..."
  ...
  try {
    ...
    if (inputsConfigObj === undefined || !inputsConfigObj) {
      console.log(
        chalkError(
          "UBI | DROPBOX LOAD BEST INPUTS CONFIG FILE ERROR | JSON UNDEFINED ??? "
        )
      );
      throw new Error(
        "DROPBOX LOAD BEST INPUTS CONFIG FILE ERROR | JSON UNDEFINED"
      );
    }
    ...
    console.log(
      chalkBlue(
        "UBI | LOADED DROPBOX BEST INPUTS CONFIG" +
          "\nTFE | CURRENT FILE BEST INPUTS IDS SET: " +
          tempInputsIdSet.size +
          " BEST INPUTS IDS" +
          "\n" +
          jsonPrint([...tempInputsIdSet]) +
          "\nTFE | FINAL BEST INPUTS IDS SET: " +
          inputsIdSet.size +
          " BEST INPUTS IDS" +
          "\n" +
    ...
    if (err.status === 409 || err.status === 404) {
      console.log(
        chalkError("UBI | DROPBOX LOAD BEST INPUTS CONFIG FILE NOT FOUND")
      );
      return;
    }
    console.log(
      chalkError("UBI | DROPBOX LOAD BEST INPUTS CONFIG FILE ERROR: ", err)
    );
    throw err;
  }
};

const initConfig = async (cnf) => {
