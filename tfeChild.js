...
DEFAULT_CONFIGURATION.processPrefix = "TFC";

DEFAULT_CONFIGURATION.childPrefix = "TFC";
...
function initDropbox(cfg){
  ...
  return new Promise(async function (resolve, reject){
    ...
    try {
      ...
      console.log(chalkError("TFC | " + configuration.childId + " | ERR INIT DROPBOX: " + err));
...
function slackPostMessage(params, callback) {
  ...
  slack.api("chat.postMessage", { text: text, channel: channel }, function(err, response) {
    if (err) {
      console.log(chalkError("TFC | " + configuration.childId + " | *** SLACK POST MESSAGE ERROR\n" + err));
    }
    else {
      debug(response);
    }
...
function initSlack(config){
  ...
  return new Promise(async function (resolve, reject){
    ...
    try {
      ...
      console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR INIT SLACK: " + err));
...
async function reset(params) {
  ...
  return new Promise(function(resolve, reject){
    ...
    setTimeout(function(){ 
      console.log(chalkAlert("TFC | " + configuration.childId + " | RESET"));
      resolve("RESET");
    }, 1000);
...
async function quit(options) {
  ...
  console.log(chalkAlert("TFC | " + configuration.childId + " | QUITTING ..." ));

  let forceQuitFlag = false;

  if (options) { 
    console.log(chalkAlert("TFC | " + configuration.childId + " | QUIT OPTIONS\n" + jsonPrint(options) ));
  ...
  if (slack) {    
    ...
    // console.log(chalkAlert( "TFC | SLACK TEXT: " + slackText));
...
function loadFile(path, file, callback) {
  ...
  return new Promise(function(resolve, reject){
    ...
    // console.log(chalkInfo("TFC | " + configuration.childId + " | LOAD FOLDER " + path));
    // console.log(chalkInfo("TFC | LOAD FILE " + file));
    console.log(chalkInfo("TFC | " + configuration.childId + " | FULL PATH " + path + "/" + file));
    ...
    if (configuration.offlineMode) {
      ...
      console.log(chalkLog("TFC | " + configuration.childId + " | " + getTimeStamp()
      ...
      dropboxClient.filesDownload({path: fullPath})
        .then(function(data){
          console.log("TFC | " + configuration.childId + " | " + chalkLog(getTimeStamp()
          ...
          if (file.match(/\.json$/gi)) {
            ...
            if (!payload || (payload === undefined)) {
              console.log(chalkError("TFC | " + configuration.childId + " | ERROR JSON PARSE"));
              return reject(new Error("TFC | " + configuration.childId + " | LOAD FILE PAYLOAD UNDEFINED"));
            }
...
function getFileMetadata(folder, file) {
  ...
  return new Promise(async function(resolve, reject){
    ...
    console.log(chalkInfo("TFC | " + configuration.childId + " | getFileMetadata FULL PATH: " + fullPath));
    ...
    try {
      ...
      if ((err.status === 404) || (err.status === 409)) {
        console.error(chalkError("TFC | " + configuration.childId + " | *** DROPBOX GET FILE METADATA | " + fullPath + " NOT FOUND"));
      }
      if (err.status === 0) {
        console.error(chalkError("TFC | " + configuration.childId + " | *** DROPBOX GET FILE METADATA | NO RESPONSE"));
      }
    ...
    try{
      ...
      if (configuration.offlineMode) {
        ...
        if (fileModifiedMoment.isSameOrBefore(prevConfigFileModifiedMoment)){
          ...
          console.log(chalkInfo("TFC | " + configuration.childId + " | CONFIG FILE BEFORE OR EQUAL"
            + " | " + fullPath
          ...
          console.log(chalkInfo("TFC | " + configuration.childId + " | +++ CONFIG FILE AFTER ... LOADING"
            + " | " + fullPath
            + " | PREV: " + prevConfigFileModifiedMoment.format(compactDateTimeFormat)
            + " | " + fileModifiedMoment.format(compactDateTimeFormat)
          ));
          ...
          try {
            ...
            console.log(chalkInfo("TFC | " + configuration.childId + " | LOADED CONFIG FILE: " + file + "\n" + jsonPrint(loadedConfigObj)));

            let newConfiguration = {};
            newConfiguration.socket = {};

            if (loadedConfigObj.UTIL_TARGET_SERVER !== undefined) {
              console.log("TFC | " + configuration.childId + " | LOADED _UTIL_TARGET_SERVER: " + loadedConfigObj.UTIL_TARGET_SERVER);
            ...
            if (loadedConfigObj.QUIT_ON_COMPLETE !== undefined) {
              console.log("TFC | LOADED QUIT_ON_COMPLETE: " + loadedConfigObj.QUIT_ON_COMPLETE);
            ...
            if (loadedConfigObj.VERBOSE !== undefined) {
              console.log("TFC | " + configuration.childId + " | LOADED VERBOSE: " + loadedConfigObj.VERBOSE);
            ...
            if (loadedConfigObj.KEEPALIVE_INTERVAL !== undefined) {
              console.log("TFC | " + configuration.childId + " | LOADED KEEPALIVE_INTERVAL: " + loadedConfigObj.KEEPALIVE_INTERVAL);
              newConfiguration.keepaliveInterval = loadedConfigObj.KEEPALIVE_INTERVAL;
            }

            if (loadedConfigObj.TWITTER_USER !== undefined) {
              console.log("TFC | " + configuration.childId + " | LOADED TWITTER_USER: " + loadedConfigObj.TWITTER_USER);
            ...
            console.log(chalkError("TFC | " + configuration.childId + " | *** LOAD FILE ERROR\n" + jsonPrint(err.error)));
            reject(err);
          }
      ...
      if ((err.status === 404) || (err.status === 409)) {
        console.error(chalkError("TFC | " + configuration.childId + " | *** DROPBOX LOAD CONFIG FILE | FILE NOT FOUND"));
      }
      else if (err.status === 0) {
        console.error(chalkError("TFC | " + configuration.childId + " | *** DROPBOX LOAD CONFIG FILE | NO RESPONSE"));
      }
      else {
        console.log(chalkError("TFC | " + configuration.childId + " | *** LOAD CONFIG FILE ERROR: " + err.error.error_summary));
      }
      reject(err);
    }
  });
...
function loadAllConfigFiles(){
  ...
  return new Promise(async function(resolve, reject){
    ...
    console.log(chalkInfo("TFC | " + configuration.childId + " | LOAD ALL DEFAULT " + configuration.defaultConfigFolder + "/" + configuration.defaultConfigFile));
    ...
    async.parallel({
      ...
      loadedDefaultConfig: function(cb){
        ...
        loadConfigFile(configuration.defaultConfigFolder, configuration.defaultConfigFile)
          ...
          .catch(function(err){
            console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR LOAD DEFAULT CONFIG: " + err.error.error_summary));
            cb();
          });
      ...
      loadedHostConfig: function(cb){
        ...
        loadConfigFile(configuration.hostConfigFolder, configuration.hostConfigFile)
          ...
          .catch(function(err){
            console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR LOAD HOST CONFIG: " + err.error.error_summary));
            cb();
          });
    ...
    function(err, results){
      if (err) {
        console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR LOAD ALL CONFIGS\n" + jsonPrint(err)));
        return reject(err);
      }
  ...
  return new Promise(async function (resolve, reject){
    ...
    if (global.dbConnection) {
      ...
      global.dbConnection.close(function () {
        ...
        console.log(chalkAlert(
          "\nTFC ==========================\n"
          + "TFC | " + configuration.childId + " MONGO DB CONNECTION CLOSED"
          + "\nTFC ==========================\n"
        ));
  ...
  "RESET":{
    ...
    onEnter: async function(event, oldState, newState) { 
      if (event !== "fsm_tick") {
        ...
        process.send({op:"RESET", childId: configuration.childId});
...
function initFsmTickInterval(interval) {
  console.log(chalkInfo("TFC | INIT FSM TICK INTERVAL | " + msToTime(interval)));
...
function showStats(options) {
  ...
  if (options) {
    ...
    console.log("TFC | " + configuration.childId + " | " + chalkLog(configuration.processPrefix
...
process.on("exit", function() {
  console.log(chalkAlert("TFC | " + configuration.childId + " | *** EXIT ***"));
  quit({source: "EXIT"});
});

process.on( "SIGHUP", function() {
  console.log(chalkAlert("TFC | " + configuration.childId + " | *** SIGHUP ***"));
  quit({source: "SIGHUP"});
});

process.on( "SIGINT", function() {
  console.log(chalkAlert("TFC | " + configuration.childId + " | *** SIGINT ***"));
  quit({source: "SIGINT"});
});

process.on("disconnect", function() {
  console.log(chalkAlert("TFC | " + configuration.childId + " | *** DISCONNECT ***"));
  quit("DISCONNECT");
});

process.on("message", async function(m) {

  debug(chalkAlert("TFC CHILD RX MESSAGE"
    + " | OP: " + m.op
  ));
  ...
  switch (m.op) {
    ...
    case "SIGINT":
      ...
      setTimeout(function() {
        console.log("TFC QUITTING | @" + configuration.childId);
        process.exit(0);
      }, 500);
    break;
    ...
    case "INIT":
      ...
      console.log(chalkInfo("TFC | TMP CHILD INIT"
        + " | CHILD ID: " + m.childId
        + " | 3C: @" + m.threeceeUser
    ...
    case "RESET_TWITTER_USER_STATE":
      console.log("TFC | " + configuration.childId 
        + " | @" + configuration.threeceeUser 
    ...
    case "VERBOSE":
      console.log(chalkInfo("TFC @" + configuration.childId + " | SET VERBOSE: " + m.verbose));
      configuration.verbose = m.verbose;
    break;

    default:
      console.log(chalkError("TFC | " + configuration.childId 
        + " | UNKNOWN OP ERROR"
        + " | " + m.op
      ));
  }
...
function saveFile (params, callback){
  ...
  if (params.localFlag) {
    ...
    showStats();
    console.log(chalkBlue("TFC | " + configuration.childId 
    ...
    .catch(function(err){
      console.trace(chalkError("TFC | " + configuration.childId 
    ...
    const dbFileUpload = function () {
      ...
      .then(function(){
        debug(chalkLog("TFC | " + configuration.childId 
      ...
      .catch(function(err){
        if (err.status === 413){
          console.error(chalkError("TFC | " + configuration.childId 
            + " | " + moment().format(compactDateTimeFormat) 
    ...
    if (options.mode === "add") {
      ...
      .then(function(response){
        ...
        async.each(response.entries, function(entry, cb){
          ...
          console.log(chalkInfo("TFC | " + configuration.childId 
            + " | DROPBOX FILE"
            + " | " + params.folder
          ...
          if (fileExits) {
            console.log(chalkAlert("TFC | " + configuration.childId 
      ...
      .catch(function(err){
        console.log(chalkError("TFC | " + configuration.childId + " | *** DROPBOX FILES LIST FOLDER ERROR: " + err));
        console.log(chalkError("TFC | " + configuration.childId + " | *** DROPBOX FILES LIST FOLDER ERROR\n" + jsonPrint(err)));
        if (callback !== undefined) { callback(err, null); }
      });
    }
    else {
    ...
    if (!saveFileBusy && saveFileQueue.length > 0) {
      ...
      saveFile(saveFileObj, function(err) {
        if (err) {
          ...
          console.log(chalkLog("TFC | " + configuration.childId 
...
function sendKeepAlive(callback) {
  ...
  if (statsObj.userAuthenticated && statsObj.serverConnected) {
    ...
    if (configuration.verbose) {
      debug(chalkInfo("TFC | " + configuration.childId + " | TX KEEPALIVE"
    ...
    console.log(chalkError("TFC | " + configuration.childId + " | !!!! CANNOT TX KEEPALIVE"
      + " | " + configuration.user.userId
...
function initKeepalive(interval) {
  ...
  console.log(chalkConnect("TFC | " + configuration.childId + " | START KEEPALIVE"
    + " | " + getTimeStamp()
    + " | READY ACK: " + statsObj.userAuthenticated
    + " | SERVER CONNECTED: " + statsObj.serverConnected
  ...
  sendKeepAlive(function(err) {
    if (err) {
      console.log(chalkError("TFC | " + configuration.childId + " | KEEPALIVE ERROR: " + err));
    }
    else if (configuration.verbose) {
      console.log(chalkLog("TFC | " + configuration.childId + " | T> KEEPALIVE | " + moment().format(compactDateTimeFormat)));
    }
  });
...
function initUserReadyInterval(interval) {
  ...
  console.log(chalkInfo("TFC | " + configuration.childId + " | INIT USER READY INTERVAL"));
  ...
  return new Promise(async function (resolve, reject){
    ...
    try {
      ...
      wordAssoDb.connect(name, function(err, db){
        ...
        if (err) {
          console.log(chalkError("TFC | " + configuration.childId + " | *** MONGO DB CONNECTION ERROR | " + name + " | " + err));
          statsObj.db.connected = false;
          return reject(err);
        }

        db.on("error", function(){
          console.log(chalkError("TFC | " + configuration.childId + " | " + name + " | *** MONGO DB CONNECTION ERROR ***\n"));
          db.close();
          statsObj.db.connected = false;
        });

        db.on("disconnected", function(){
          console.log(chalkAlert("TFC | " + configuration.childId + " | " + name + " | *** MONGO DISCONNECTED ***\n"));
          statsObj.db.connected = false;
        });

        console.log(chalk.green("TFC | " + configuration.childId + " | " + name + " | MONGOOSE DEFAULT CONNECTION OPEN"));
        ...
        async.parallel({
          countUsers: function(cb){
            User.countDocuments({}, function (err, count) {
              if (err) {
                console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR INIT DB | COUNT USERS: " + err));
                return cb(err);
              }
              console.log(chalkInfo("TFC | " + configuration.childId + " | " + count + " USERS"));
              cb();
            });
          },
          countHashtags: function(cb){
            Hashtag.countDocuments({}, function (err, count) {
              ...
              console.log(chalkInfo("TFC | " + configuration.childId + " | " + count + " HASHTAGS"));
              cb();
            });
          },
          countNeuralNetworks: function(cb){
            NeuralNetwork.countDocuments({}, function (err, count) {
              if (err) {
                console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR INIT DB | COUNT NEURAL NETWORKS: " + err));
                return cb(err);
              }
              console.log(chalkInfo("TFC | " + configuration.childId + " | " + count + " NEURAL NETWORKS"));
              cb();
            });
          },
        },
      ...
      console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR INIT DB: " + err));
      statsObj.db.err = err;
...
function initSocket(config){
  ...
  return new Promise(async function (resolve, reject){
    ...
    if (configuration.offlineMode) {
      console.log(chalkError("TFC | " + configuration.childId + " | *** INIT SOCKET | OFFLINE MODE *** "));
      return reject(new Error("TFC | OFFLINE MODE"));
    }

    if (socket && socket.id) {
      console.log(chalkAlert("TFC | " + configuration.childId + " | !!! INIT SOCKET | DISCONNECT EXISTING SOCKET CONNECTION | " + socket.id));
      socket.disconnect();
      socket = null;
    }
    ...
    try {
      ...
      console.log(chalkLog("TFC | " + configuration.childId + " | INIT SOCKET"
        + " | " + targetServer
        + " | USER: " + user.userId
      ...
      s.on("connect", function() {
        ...
        console.log(chalkConnect("TFC | " + configuration.childId + " | SOCKET CONNECT | " + s.id + " ... AUTHENTICATE ..."));

        s.emit("authentication", { namespace: "util", userId: user.userId, password: "0123456789" });
      });

      s.on("unauthorized", function(err) {
        console.log(chalkError("TFC | *** AUTHENTICATION ERROR: ", err.message));
        statsObj.userAuthenticated = false ;
      });
      ...
      s.on("authenticated", function() {
        ...
        console.log("TFC | " + configuration.childId + " | SOCKET AUTHENTICATED | " + s.id);

        statsObj.socketId = socket.id;

        console.log(chalkConnect("TFC | " + configuration.childId + " | SOCKET CONNECTED TO SERVER"
        ...
        console.log(chalkInfo("TFC | " + configuration.childId + " | T> USER_READY"
          + " | " + s.id
          + " | " + moment().format(compactDateTimeFormat)
          + " | " + user.userId
          + " | " + user.url
      ...
      s.on("disconnect", function(reason) {
        ...
        console.log(chalkAlert("TFC | " + moment().format(compactDateTimeFormat)
      ...
      s.on("reconnect", function(reason) {
        ...
        console.log(chalkInfo("TFC | " + configuration.childId + " | SOCKET RECONNECT"
          + " | " + moment().format(compactDateTimeFormat)
          + " | " + s.id
          + " | REASON: " + reason
        ));
      ...
      s.on("USER_READY_ACK", function(user) {
        ...
        console.log(chalkBlue("TFC | " + configuration.childId + " | R< USER_READY_ACK MESSAGE"
          + " | " + s.id
          + " | USER ID: " + user.userId
          + " | " + moment().format(compactDateTimeFormat)
        ));
      ...
      s.on("error", function(err) {
        console.log(chalkError("TFC | " + configuration.childId + " | " + moment().format(compactDateTimeFormat)
          + " | *** SOCKET ERROR"
          + " | " + s.id
          + " | " + err
        ));
      ...
      s.on("connect_error", function(err) {
        ...
        console.log(chalkError("TFC | " + configuration.childId + " | *** SOCKET CONNECT ERROR "
          + " | " + moment().format(compactDateTimeFormat)
          + " | " + err.type
          + " | " + err.description
        ));
      ...
      s.on("reconnect_error", function(err) {
        ...
        console.log(chalkError("TFC | " + configuration.childId + " | *** SOCKET RECONNECT ERROR "
          + " | " + moment().format(compactDateTimeFormat)
          + " | " + err.type
          + " | " + err.description
        ));
      ...
      s.on("HEARTBEAT", function() {
        ...
        if (configuration.verbose) { console.log(chalkLog("TFC | " + configuration.childId + " | R< HEARTBEAT [" + statsObj.heartbeatsReceived + "]")); }
      });
      ...
      console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR INIT SOCKET: " + err));
...
function initTwitter(twitterConfig){
  ...
  return new Promise(async function (resolve, reject){
    ...
    if (configuration.offlineMode) {
      console.log(chalkError("TFC | " + configuration.childId + " | *** INIT TWITTER | OFFLINE MODE *** "));
      return reject(new Error("OFFLINE MODE"));
    }

    if (twit && twitterConfig.reinit) {
      console.log(chalkAlert("TFC | " + configuration.childId + " | !!! INIT TWITTER | DISCONNECT EXISTING TWITTER CONNECTION"));
      // twit.disconnect();
      twit = null;
    }
    else if (twit) {
    ...
    try {
      ...
      console.log(chalkLog("TFC | " + configuration.childId + " | INIT TWITTER"));
      ...
      t.get("account/verify_credentials", { skip_status: true })
        ...
        .then(function (result) {
          // console.log(chalkLog("TFC | INIT TWITTER RESULTS\n" + jsonPrint(result.data)));
          resolve(t);
        });
    }
    catch (err) {
      console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR INIT TWITTER: " + err));
...
function initStatsUpdate(callback) {
  ...
  console.log(chalkTwitter("TFC | " + configuration.childId + " | INIT STATS UPDATE INTERVAL | " + configuration.statsUpdateIntervalTime + " MS"));
...
function toggleVerbose(){
  ...
  console.log(chalkAlert("TFC | " + configuration.childId + " | VERBOSE: " + configuration.verbose));
}
...
function initConfig(cnf) {
  ...
  return new Promise(async function(resolve, reject){
    ...
    try {
      ...
      console.log(chalkInfo("TFC | " + configuration.childId + " | INITIALIZING..."));
      if (cnf.verbose) { console.log(chalkInfo("TFC | CONFIGURATION\n" + jsonPrint(cnf) ));}
      ...
      if (cnf.testMode) {
        console.log(chalkAlert("TFC | " + configuration.childId + " | TEST MODE"));
      }
      ...
      console.log(chalkError("TFC | " + configuration.childId + " | *** ERROR INIT CONFIG: " + err));
...
function startFsmMain(){
  ...
  console.log(chalkInfo("TFC | +++ START FSM CHILD | " + configuration.childId + " | " + getTimeStamp()));
  fsmMain["fsm_reset"]();
}
...
initConfig(configuration)
  .then(async function(cnf){
    ...
    console.log(chalkTwitter("TFC | " + configuration.childId
  ...
  .catch(function(err){
    console.log(chalkError("TFC | " + configuration.childId + " | ***** INIT CONFIG ERROR ***** ", err));
    quit();
  });